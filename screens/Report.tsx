...
const ReportPollution: React.FC<Props> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.description) return alert("Please provide details.");
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center p-10 text-center min-h-screen animate-in zoom-in duration-500 bg-[#020617] text-white">
        ...
        <h2 className="text-4xl font-black tracking-tighter">Evidence Secured!</h2>
        <p className="text-cyan-100/60 mt-4 leading-relaxed max-w-[85%] mx-auto font-medium">
          Saved to local ecosystem vault. You've earned 
        ...
        <div className="mt-12 w-full space-y-4 px-6">
          <button 
            ...
            className="w-full py-5 bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-black rounded-[2rem] shadow-2xl transition-all active:scale-95"
          >
            Return to Dashboard
          </button>
        </div>
    ...
    <div className="pb-32 bg-[#f8fafc] min-h-screen">
      ...
      <div className="relative h-[300px] overflow-hidden bg-[#083344]">
        ...
        <div className="relative z-10 pt-16 px-8">
          ...
          <p className="text-cyan-100/60 text-sm mt-2 font-medium">Data remains private on your device.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="px-6 -mt-16 space-y-8 relative z-20">
        <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-slate-100 space-y-8">
              ...
              {[PollutionType.PLASTIC, PollutionType.OIL, PollutionType.CHEMICAL, PollutionType.OTHER].map(type => (
                <button
                  ...
                  className={`py-4 px-2 rounded-2xl text-[10px] font-black transition-all border-2 ${
                    formData.type === type ? 'bg-cyan-50 border-cyan-500 text-cyan-600 shadow-lg' : 'bg-slate-50 border-slate-50 text-slate-400'
                  }`}
                >
                  {type.toUpperCase()}
                </button>
              ...
              {Object.values(Severity).map(s => (
                <button
                  ...
                  className={`w-12 h-12 rounded-full font-black text-[8px] transition-all flex items-center justify-center ${
                    formData.severity === s ? getSeverityColor(s) + ' shadow-xl scale-110' : 'text-slate-300'
                  }`}
                >
                  {s.charAt(0)}
                </button>
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <input readOnly value={formData.location} className="w-full p-5 bg-slate-50 border-0 rounded-3xl outline-none pr-16 font-bold text-slate-600 text-sm" />
              <button type="button" onClick={handleGPS} disabled={loading} className={`absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-2xl text-white shadow-lg active:scale-90 transition-all flex items-center justify-center ${loading ? 'bg-slate-300 animate-pulse' : 'bg-cyan-600'}`}>
          ...
          <div className="space-y-4">
            ...
            <textarea rows={4} placeholder="What did you observe?..." value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} className="w-full p-6 bg-slate-50 border-0 rounded-[2rem] outline-none resize-none font-medium text-slate-700 text-sm" />
          </div>

          <div className="flex space-x-4">
            <button type="button" onClick={handleImageClick} className="w-28 h-28 bg-cyan-50 border-2 border-dashed border-cyan-200 rounded-[2.5rem] flex flex-col items-center justify-center text-cyan-600 transition-all active:scale-95 group overflow-hidden relative">
              {formData.image ? (
                <>
                  <img src={formData.image} alt="Evidence preview" className="w-full h-full object-cover" />
                  <button onClick={removeImage} className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg active:scale-75"><ICONS.Zap size={12} className="rotate-45" /></button>
                </>
              ) : (
                <>
        ...
        <button disabled={loading || formData.location === 'Awaiting GPS...'} type="submit" className={`w-full py-6 text-white text-sm font-black rounded-[2.5rem] shadow-2xl transition-all active:scale-95 ${loading || formData.location === 'Awaiting GPS...' ? 'bg-slate-300' : 'bg-gradient-to-r from-cyan-600 to-blue-700'}`}>
          {loading ? 'STORING EVIDENCE...' : 'LOG EVIDENCE (+50 XP)'}
        </button>
      </form>
    </div>
  );
